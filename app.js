(() => {
  ...
  const FIELD_CONFIG = [
    ...
    { name: 'Link', title: 'LINK / LAMPIRAN', type: 'Attachments', optional: true, description: 'Kolum jenis Attachments yang menyimpan fail PDF/dokumen dalam Grist.' },
  ...
  function extractAttachmentIds(value) {
    ...
    if (Array.isArray(value)) {
      if (value[0] === 'L') {
        // Nilai mentah Grist: ["L", attachmentId, ...]
        items = value.slice(1);
      } else if (value[0] === 'r' && value[1] === '_grist_Attachments' && Array.isArray(value[2])) {
        // Format typed: ["r", "_grist_Attachments", [attachmentId, ...]]
        items = value[2];
      } else {
        // Nilai yang telah didekod oleh Custom Widget API: [attachmentId, ...]
        items = value;
      }
    } else if (typeof value === 'number') {
      items = [value];
    } else if (typeof value === 'object') {
      const candidate = value.attachmentId ?? value.attachment_id ?? value.id;
      if (candidate !== undefined) items = [candidate];
    }
  ...
  function urlCandidates(value, output = []) {
    if (value === null || value === undefined || value === '') return output;

    if (Array.isArray(value)) {
      const code = value[0];
      const items = typeof code === 'string' && code.length === 1 ? value.slice(1) : value;
      items.forEach(item => urlCandidates(item, output));
      return output;
    }

    if (typeof value === 'object') {
      ['url', 'href', 'link', 'value', 'display', 'text', 'name'].forEach(key => {
        if (value[key] !== undefined) urlCandidates(value[key], output);
      });
      return output;
    }
    ...
    // Format Markdown: [Buka dokumen](https://...)
    const markdown = text.match(/\[[^\]]*\]\((https?:\/\/[^\s)]+)\)/i);
    if (markdown) output.push(markdown[1]);

    // Format HTML: <a href="https://...">...</a>
    const htmlHref = text.match(/href\s*=\s*["'](https?:\/\/[^"']+)["']/i);
    if (htmlHref) output.push(htmlHref[1]);

    // Format hyperlink Grist: "Nama Dokumen https://...".
    // Grist menggunakan bahagian terakhir selepas ruang sebagai URL.
    const parts = text.split(/\s+/).filter(Boolean);
    if (parts.length > 1) output.push(parts[parts.length - 1]);

    // URL biasa yang muncul di mana-mana dalam teks.
  ...
  function normalizeUrlCandidate(candidate) {
    let text = decodeHtmlEntities(candidate).trim();
    if (!text) return '';
  ...
  function openExternalUrl(value) {
    ...
    if (!url) {
      showToast('Pautan dokumen tidak sah atau belum diisi.');
      return;
    }
  ...
  function loadingPopup() {
    ...
    try {
      popup.document.title = 'Memuatkan dokumen...';
      popup.document.body.innerHTML = '<p style="font-family:Arial,sans-serif;padding:24px;color:#2C3240">Memuatkan dokumen daripada Grist...</p>';
    } catch {
      // Sesetengah pelayar tidak membenarkan penulisan ke tetingkap sementara.
    }
    return popup;
  }

  async function openGristAttachment(attachmentId) {
    if (!Number.isInteger(Number(attachmentId)) || Number(attachmentId) <= 0) {
      showToast('ID lampiran Grist tidak sah.');
      return;
    }

    // Buka tetingkap ketika klik pengguna masih aktif supaya tidak disekat sebagai pop-up.
    const popup = loadingPopup();

    try {
      if (!window.grist?.docApi?.getAccessToken) {
        throw new Error('API token Grist tidak tersedia.');
      }
      const tokenInfo = await window.grist.docApi.getAccessToken({ readOnly: true });
      const baseUrl = String(tokenInfo.baseUrl || '').replace(/\/$/, '');
      const url = `${baseUrl}/attachments/${encodeURIComponent(attachmentId)}/download?auth=${encodeURIComponent(tokenInfo.token)}`;

      if (popup && !popup.closed) {
        popup.location.replace(url);
      } else {
        // Sandaran jika pop-up disekat.
        const anchor = document.createElement('a');
        anchor.href = url;
        anchor.target = '_blank';
        anchor.rel = 'noopener noreferrer';
      ...
      console.error('Gagal membuka lampiran Grist:', error);
      if (popup && !popup.closed) popup.close();
      showToast('Lampiran tidak dapat dibuka. Semak akses widget dan pastikan fail masih wujud dalam Grist.');
    }
  }

  function openRecordDocument(record, attachmentIndex = 0) {
    if (!record) return;
    if (record.attachmentIds?.length) {
      const id = record.attachmentIds[Math.max(0, Number(attachmentIndex) || 0)] || record.attachmentIds[0];
      openGristAttachment(id);
      return;
    }
    if (record.link) {
      openExternalUrl(record.link);
      return;
    }
    showToast('Tiada lampiran atau pautan bagi rekod ini.');
  }
  ...
  function normalizeRecord(record) {
    ...
    const attachmentIds = extractAttachmentIds(record.Link);
    const externalUrl = attachmentIds.length ? '' : extractUrl(record.Link);
    const normalized = {
      ...
      attachmentIds,
      link: externalUrl,
      linkRaw: attachmentIds.length ? `Lampiran Grist (${attachmentIds.length})` : safeText(record.Link),
      hasDocument: attachmentIds.length > 0 || Boolean(externalUrl),
  ...
  function renderTable() {
    ...
    $('tableBody').innerHTML = records.map((record, index) => {
      ...
      return `<tr data-row-index="${start + index}">
        ...
        <td>${tags}</td>
        <td>${record.hasDocument ? `<button class="link-button open-document" type="button" data-open-record="${start + index}" aria-label="Buka lampiran atau dokumen dalam tab baharu">${record.attachmentIds.length > 1 ? `Buka (${record.attachmentIds.length}) ↗` : 'Buka ↗'}</button>` : '<span class="no-link">Tiada lampiran</span>'}</td>
      </tr>`;
    }).join('');
  }
  ...
  function showDetail(record) {
    ...
    $('detailContent').innerHTML = `
      ...
      <div class="detail-actions">
        ${record.hasDocument ? `<button class="button button-primary open-document" type="button" data-open-detail> ${record.attachmentIds.length > 1 ? `Buka Lampiran Pertama (${record.attachmentIds.length} fail) ↗` : 'Buka Dokumen ↗'}</button>` : ''}
    ...
    $('detailContent').querySelector('[data-open-detail]')?.addEventListener('click', event => {
      event.preventDefault();
      openRecordDocument(record);
    });
  ...
  function bindEvents() {
    ...
    $('tableBody').addEventListener('click', event => {
      const documentButton = event.target.closest('[data-open-record]');
      if (documentButton) {
        event.preventDefault();
        event.stopPropagation();
        openRecordDocument(state.filtered[Number(documentButton.dataset.openRecord)]);
        return;
      }
  ...
  function showMappingMessage() {
    ...
    $('mappingMessage').textContent = 'Padankan kolum ID, KATEGORI, SUB KATEGORI, TAJUK, SIPNOPSIS, LINK (jenis Attachments), JABATAN, KELULUSAN, TARIKH dan HASHTAG.';
    $('loadingOverlay').classList.add('hidden');
  }
  const demoRecords = [
    { id:1, ID:'GP-001', Kategori:'PIAWAIAN PERANCANGAN', SubKategori:'PERUMAHAN', Tajuk:'Garis Panduan Piawaian Perumahan Majlis Bandaraya Seberang Perai', Sinopsis:'Panduan umum perancangan perumahan tidak berstrata, berstrata dan komuniti berpagar meliputi jalan, anjakan, kawasan lapang, kemudahan masyarakat dan tempat letak kenderaan.', Link:'Dokumen Contoh https://www.w3.org/WAI/ER/tests/xhtml/testfiles/resources/pdf/dummy.pdf', Jabatan:'Jabatan Perancang Bandar MBSP', Kelulusan:'Perlu Pengesahan', Tarikh:'2021-12-08', Hashtag:['Perumahan','Berstrata','GatedCommunity','KemudahanMasyarakat','TLK'] },
