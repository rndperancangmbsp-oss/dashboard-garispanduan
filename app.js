(() => {
  ...
  function openDocumentUrl(value) {
    ...
    try {
      // Gunakan klik pautan sebenar dalam tindakan pengguna supaya serasi dengan iframe Grist.
      ...
      showToast('Dokumen tidak dapat dibuka. Semak pautan atau tetapan pop-up pelayar.');
    }
  }
  ...
  function renderTable() {
    ...
    $('tableBody').innerHTML = records.map((record, index) => {
      ...
      return `<tr data-row-index="${start + index}">
        ...
        <td>${tags}</td>
        <td>${record.link ? `<button class="link-button open-document" type="button" data-open-document="${escapeHtml(record.link)}" aria-label="Buka dokumen dalam tab baharu">Buka ↗</button>` : '<span class="no-link">Tiada pautan</span>'}</td>
      </tr>`;
    }).join('');
  }
  ...
  function showDetail(record) {
    ...
    $('detailContent').innerHTML = `
      ...
      <div class="detail-actions">
        ${record.link ? `<button class="button button-primary open-document" type="button" data-open-document="${escapeHtml(record.link)}">Buka Dokumen ↗</button>` : ''}
    ...
    $('detailContent').querySelector('[data-open-document]')?.addEventListener('click', event => {
      event.preventDefault();
      openDocumentUrl(event.currentTarget.dataset.openDocument);
    });
  ...
  function bindEvents() {
    ...
    $('tableBody').addEventListener('click', event => {
      ...
      if (documentButton) {
        ...
        openDocumentUrl(documentButton.dataset.openDocument);
        return;
      }
